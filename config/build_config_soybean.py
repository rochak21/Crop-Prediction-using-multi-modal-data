...
def get_json_obj(year, county_info):
    ...
    obj = {
        ...
        "data": {
            ...
            # "USDA": "USDA Crop Dataset/Soybean/{}/USDA_Soybean_County_{}.csv".format(year, year),
              "USDA": "USDA Crop Dataset/Soybean/{}/USDA Crop Dataset_data_Soybean_{}_USDA_Soybean_County_{}.csv".format(year,year,year),



                "sentinel": [
                "Sentinel-2/{}/{}/Agriculture_{}_{}_{}-04-01_{}-06-30.h5".format(year, state, fips[:2],
                                                                                              state, year, year),
                "Sentinel-2/{}/{}/Agriculture_{}_{}_{}-07-01_{}-09-30.h5".format(year, state, fips[:2],
                                                                                              state, year, year),
            ]
        }
...
def get_short_HRRR_obj(state, fips, year, months=[i + 1 for i in range(12)]):
    ...
    for month in months:
        ...
        # Correct path generation to match the file naming structure
        path = f"WRF-HRRR/{year}/{state}/WRF-HRRR Computed Dataset_data_{year}_{state}_HRRR_{fips[:2]}_{state}_{year}-{month}.csv"
