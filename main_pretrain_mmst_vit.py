...
def main(args):
    ...
    # Use CPU or CUDA based on the --device argument
    device = torch.device(args.device)
    if 'cuda' in str(device) and not torch.cuda.is_available():
        raise RuntimeError("CUDA device requested but not available. Check your setup.")
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (x, y) in enumerate(zip(data_loader_sentinel, data_loader_hrrr)):
        ...
        fips = x[1][0]
        max_mem = torch.cuda.max_memory_allocated() / (1024.0 * 1024.0) if device.type == 'cuda' else 0
