    ...
    def __init__(self, root_dir, json_file, num_workers=4):
        ...
        for obj in data:
            ...
            self.years.append(obj["year"])
                ...
                # No path modification, assume the paths are correct from JSON
            short_term = [os.path.join(root_dir, file_path) for file_path in obj["data"]["HRRR"]["short_term"]]
            long_term = [[os.path.join(root_dir, file_path) for file_path in file_paths] for file_paths in
                             obj["data"]["HRRR"]["long_term"]]
