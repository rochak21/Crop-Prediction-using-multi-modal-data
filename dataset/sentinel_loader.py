    ...
    def __init__(self, root_dir, json_file):
        ...
        for obj in data:
            ...
            for relative_path in relative_path_list:
                # Construct the file path using root_dir
                full_path = os.path.join(root_dir, relative_path)
                tmp_path.append(full_path)
                # Debug print to verify constructed paths
    ...
    def __getitem__(self, index):
        ...
        for file_path in file_paths:
            if not os.path.exists(file_path):
                ...
                continue  # Skip missing files or handle as needed
        ...
        if not temporal_list:
            raise FileNotFoundError(f"No valid files found for index {index}")
