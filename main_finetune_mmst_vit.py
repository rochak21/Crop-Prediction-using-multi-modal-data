...
def get_args_parser():
    ...
    # dataset
    parser.add_argument('-dr', '--root_dir', type=str, default='data/Tiny CropNet')
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        evaluate(model, data_loader_sentinel_val, data_loader_hrrr_val, data_loader_usda_val, device, args=args,
                 epoch=epoch)
    ...
    print(f"Best model was saved at epoch {best_epoch} with metrics: RMSE: {best_metrics[0]:.4f}, "
          f"R2: {best_metrics[1]:.4f}, Corr: {best_metrics[2]:.4f}")
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (x, y, z) in enumerate(zip(data_loader_sentinel, data_loader_hrrr, data_loader_usda)):
        ...
        fips, max_mem = x[1][0], torch.cuda.max_memory_allocated() / (1024.0 * 1024.0) if torch.cuda.is_available() else 0
...
# Global variable to track the best metrics and the corresponding epoch
best_metrics = [float("inf"), 0, 0]  # [RMSE, R2, Correlation]
best_epoch = -1  # Tracks the epoch for the best model


def save_best_model(model, epoch, rmse, r2, corr, args):
    """
    Saves the model with the best performance based on RMSE, R2, and Correlation.
    """
    global best_metrics, best_epoch
    if rmse < best_metrics[0]:  # Save the model only if RMSE improves
        best_metrics = [rmse, r2, corr]
        best_epoch = epoch
        if args.output_dir:
            best_model_path = os.path.join(args.output_dir, "best_fine_tune_model.pth")
            torch.save(model.state_dict(), best_model_path)
            print(f"New best model saved at epoch {epoch} with RMSE: {rmse:.4f}, R2: {r2:.4f}, Corr: {corr:.4f}")


@torch.no_grad()
def evaluate(model: torch.nn.Module, data_loader_sentinel: Iterable, data_loader_hrrr: Iterable,
             data_loader_usda: Iterable, device: torch.device, args=None, epoch=None):
    # Data augmentation by following SimCLR
    ...
    print(f"Metrics: RMSE: {rmse:.4f}, R2: {r2:.4f}, Corr: {corr:.4f}")

    # Save the best model
    if args and epoch is not None:
        save_best_model(model, epoch, rmse, r2, corr, args)
